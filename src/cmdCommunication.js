...
/**
 * cmd communication steps
 * 1、push cmd to cmdQueue
 * 2、cmdQueue processes the cmd
 */
...
function CmdCommunication(port, dispatcher) {
  ...
  this._cs = State.idle;                   //the CmdCommunication state
...
CmdCommunication.prototype._parseData = function (data) {
  ...
  if (this._cs === State.idle) {
    console.log('nbiot module receive data when IDLE : ' + data);
    return;
  }
  this.emit("responseDone", null, data);
...
CmdCommunication.prototype._getResponse = function (callback) {
  ...
  var timerHandle = setTimeout(function () {
    responseDoneCleanup(new Error('nbiot module Response Timeout'));
  }, RESPONSE_TIMEOUT);
  ...
  function responseDoneCleanup(error, response) {
    ...
    that._cs = State.idle;
    callback(error, response ? response : null);
  }
